...
def main(n_layers, max_epoch, the_file, choice: str, batch_size=20, the_elem=500):
    ...
    if choice == 'unparametrized':
        init_params = 0.01 * torch.randn(n_layers * (m + n + 2))
        ...
        init_params = 0.01 * torch.randn(n_layers * (m + n + 2) + l)
        init_params.requires_grad = True

    final_params = train_qgnn(training_loader, validation_loader, init_params, max_epoch, n_layers, choice)
...
batch = 20
elements = 500
file = '../data/dataset/QED_data_e_annih_mu.csv'
...
params = main(num_layers, num_epoch, file, feature_map, batch, elements)